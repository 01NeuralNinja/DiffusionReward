...
class ControlLDM(nn.Module):
    ...
    def __init__(
        ...
        self.controlnet = ControlNet(**controlnet_cfg)  # 可训练ControlNet
        self.fixed_controlnet = ControlNet(**controlnet_cfg)  # 固定ControlNet
        self.scale_factor = latent_scale_factor
        self.control_scales = [1.0] * 13
        self.reg_lambda = 1e-4  # 正则化强度
    ...
    def forward(self, x_noisy, t, cond):
        ...
        # 修改：仅在训练模式下计算固定ControlNet输出和正则化损失
        if self.training:
            with torch.no_grad():
                fixed_control = self.fixed_controlnet(
                    x=x_noisy, hint=c_img, timesteps=t, context=c_txt
                )
                fixed_control = [c * scale for c, scale in zip(fixed_control, self.control_scales)]
            # 计算正则化损失
            reg_loss = 0.0
            for c_train, c_fixed in zip(control, fixed_control):
                reg_loss += torch.mean((c_train - c_fixed) ** 2)
    ...
    # def setup_lora(self, lora_rank: int):
    #     l_target_modules = []
    #     l_grep = ["to_q", "to_k", "to_v", "to_out.0", "zero_convs"]
    ...
    #     """
    #     Load pre-trained LoRA weights into the ControlNet model.
    #     - Requires ControlNet to be already wrapped with PEFT.
    #     - Loads the adapter from the given path.
    #     """
    #     if not hasattr(self.controlnet, "peft_config"):
    #         raise ValueError("ControlNet is not wrapped with PEFT. Please call setup_lora first.")
    #     # Load the adapter (assuming it's saved as an adapter)
    #     self.controlnet.load_adapter(adapter_path)
    #     # Set the loaded adapter as active
    #     self.controlnet.active_adapter = "default"
...
# )
# control_ldm.setup_lora(lora_config)
# control_ldm.load_lora_weights("path/to/lora_adapter")
# control_ldm.controlnet.save_adapter("path/to/save_lora_adapter")