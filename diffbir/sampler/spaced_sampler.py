...
class SpacedSampler(Sampler):
    ...
    def sample_direct_x0(
            self,
            model: ControlLDM,
            device: str,
            t: int,  # t在0到steps-1之间，是采样时间步的索引
            steps: int,
            x_size: Tuple[int],
            cond: Dict[str, torch.Tensor],
            uncond: Dict[str, torch.Tensor],
        ...
        # 使用t作为索引直接获取对应的实际时间步
        model_t = torch.full((bs,), timesteps[t], device=device, dtype=torch.long)

        # 计算position_t - 这是反转后的索引
        position_t = torch.full((bs,), total_steps - t - 1, device=device, dtype=torch.long)

        # 获取当前时间步的CFG缩放比例
        cur_cfg_scale = self.get_cfg_scale(cfg_scale, timesteps[t])

        # 应用模型获取预测
        model_output = self.apply_model(model, x_T, model_t, cond, uncond, cur_cfg_scale)

        # 直接预测x0
        if self.parameterization == "eps":
            pred_x0 = self._predict_xstart_from_eps(x_T, position_t, model_output)
        else:
            pred_x0 = self._predict_xstart_from_v(x_T, position_t, model_output)
